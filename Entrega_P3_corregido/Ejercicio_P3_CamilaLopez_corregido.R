...
rm(list=ls())
getwd()
#setwd("C:/Users/Fernando Cabrera/Desktop/Cami_Labo/Entregas/Entrega_P3/")
...
estaciones_cercanas()
############################ ej 2 parte 2 corregido #######################
estaciones_cercanas <- function(estaciones, lat_max, lat_min, lon_max, lon_min) {
  results <- character(0) 
  for (i in 1:length(estaciones)) {
    latitud <- estaciones[[i]][[2]]
    longitud <- estaciones[[i]][[3]]
    if (latitud <= lat_max && latitud >= lat_min && longitud <= lon_max && longitud >= lon_min) {
      results <- c(results, paste("La estacion", estaciones[[i]][1], "tiene estaciones cercanas"))
    } 
    if (length(results) > 0) {
      return(results)
    } else {
...
estaciones_cercanas(resultados,-36.8,-38,-59.8,-59.9)
###############################################################################
#Parte 2 ej 3
